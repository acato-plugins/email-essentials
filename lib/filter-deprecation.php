...
add_filter( 'acato_email_essentials_mail_is_throttled', function ( $is_throttled, $ip, $mails_recently_sent ) {
...
add_filter( 'acato_email_essentials_css', function ( $css, &$mailer ) {
	// First check for 5.0.0 deprecated filter.
	$intermediate = \Acato\Email_Essentials\Plugin::apply_filters_deprecated( 'wpes_css', [
		$css,
		&$mailer,
	], '5.0.0', 'acato_email_essentials_css' );
	// Then check for 6.0.0 deprecated filter.
	$intermediate = \Acato\Email_Essentials\Plugin::apply_filters_deprecated( 'email_essentials_css', [
		$intermediate,
		&$mailer,
	], '6.0.0', 'acato_email_essentials_css' );

	return $intermediate;
}, -12345, 2 );

add_filter( 'acato_email_essentials_subject', function ( $subject, &$mailer ) {
	// First check for 5.0.0 deprecated filter.
	$intermediate = \Acato\Email_Essentials\Plugin::apply_filters_deprecated( 'wpes_subject', [
		$subject,
		&$mailer,
	], '5.0.0', 'acato_email_essentials_subject' );
	// Then check for 6.0.0 deprecated filter.
	$intermediate = \Acato\Email_Essentials\Plugin::apply_filters_deprecated( 'email_essentials_subject', [
		$intermediate,
		&$mailer,
	], '6.0.0', 'acato_email_essentials_subject' );

	return $intermediate;
}, -12345, 2 );

add_filter( 'acato_email_essentials_body', function ( $should_be_html, &$mailer ) {
	// First check for 5.0.0 deprecated filter.
	$intermediate = \Acato\Email_Essentials\Plugin::apply_filters_deprecated(
		'wpes_body',
		[
...
add_filter( 'acato_email_essentials_head', function ( $head, &$mailer ) {
	// First check for 5.0.0 deprecated filter.
	$intermediate = \Acato\Email_Essentials\Plugin::apply_filters_deprecated(
		'wpes_head',
		[
...
add_filter( 'acato_email_essentials_defaults', function ( $defaults ) {
	// First check for 5.0.0 deprecated filter.
...
add_filter( 'acato_email_essentials_settings', function ( $settings ) {
	// First check for 5.0.0 deprecated filter.
...
add_filter( 'acato_email_essentials_mail_throttle_time_window', function ( $time_window ) {
	// First check for 5.0.0 deprecated filter.
...
add_filter( 'acato_email_essentials_mail_throttle_max_count_per_time_window', function ( $count ) {
	// First check for 5.0.0 deprecated filter.
...
add_filter( 'acato_email_essentials_mail_throttle_batch_size', function ( $size ) {
	// First check for 5.0.0 deprecated filter.
...
add_filter( 'acato_email_essentials_minify_css', function ( $css ) {
...
add_filter( 'acato_email_essentials_ip_service', function ( $service, $type ) {
	// Check for 6.0.0 deprecated filter (no 5.0.0 version existed).
	$intermediate = \Acato\Email_Essentials\Plugin::apply_filters_deprecated( 'email_essentials_ip_service', [
		$service,
		$type,
	], '6.0.0', 'acato_email_essentials_ip_service' );

	return $intermediate;
}, -12345, 2 );

add_filter( 'acato_email_essentials_website_root_path', function ( $root_path ) {
	// Check for 6.0.0 deprecated filter (no 5.0.0 version existed).
...
add_filter( 'acato_email_essentials_development_tlds', function ( $tlds ) {
